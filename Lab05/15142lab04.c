...
//Method checkType() returns 1 if the input type is correct otherwise 0
int checkType(char input[]){
...
//Method checkInput() returns 1 if the input is correct otherwise 0
int checkInput(char input[]){
	int i;
	for(i = 0;i <= 8;i++){
...
void donut(char c1[],char c2[],float input){
	int i,j,k;
	float x = input/4;
	float smallRad = round(x);

	for(i = 0;i<2*smallRad;i++){
		if(i!=0){
			for(j = 0; j<2*smallRad ; j++){
				if(j!=0){
					int y = ( (2*smallRad-j)*(2*smallRad-j) ) + ((2*smallRad-i)*(2*smallRad-i));

					if(y <= (smallRad)*(smallRad)){
						textcolor(BRIGHT,checkInput(c2),checkInput(c1));
						printf(" ");
					}else if(smallRad*smallRad < y && y > (2*smallRad)*(2*smallRad)){
						textcolor(BRIGHT,checkInput(c2),checkInput(c1));
						printf(" ");
					}else{
